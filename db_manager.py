...
class TrainingDatabase:
    ...
    - user_challenges: Per-visitor challenge progress
    """
    ...
    def _init_db(self):
        ...
        with self._get_connection() as conn:
            ...
            if "visitor_id" not in session_columns:
                cursor.execute("ALTER TABLE sessions ADD COLUMN visitor_id INTEGER")
    ...
    def create_session(
        ...
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO sessions (game_id, device, hyperparameters, status, visitor_id)
                VALUES (?, ?, ?, 'running', ?)
            """, (game_id, device, json.dumps(hyperparameters or {}), visitor_id))
            conn.commit()
            session_id = cursor.lastrowid
            logger.info(f"Created session {session_id} for {game_id}")
            if visitor_id:
                self.increment_visitor_sessions(visitor_id)
            return session_id
    ...
    def get_conversion_funnel(self) -> Dict:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                SELECT COUNT(DISTINCT visitor_id) as count
                FROM analytics_events
                WHERE event_type IN ('training_started', 'training_start')
            """)
