/**
 * Training Loss Chart - Styled to match requested UI
 */
...
function LossChart({ episodes }) {
  ...
  const dataValues = trimmed.map(e => e.loss || 0).concat(padding);
  ...
  return (
    <Card sx={cardSx}>
      <MDBox px={2.5} py={2} display="flex" alignItems="center" justifyContent="space-between">
        ...
        <Tooltip
          title="Loss indicates prediction error. Lower is better."
          arrow
          componentsProps={{ tooltip: { sx: tooltipSx }, arrow: { sx: { color: tooltipSx.backgroundColor } } }}
        >
          <Icon sx={{ fontSize: '1rem !important', color: '#f59e0b', cursor: 'help' }}>
            trending_down
          </Icon>
        </Tooltip>
      </MDBox>
